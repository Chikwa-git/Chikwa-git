Hi! I'm Lincoln 👋

I'm transitioning my career from Foreign Trade to Software Development, currently studying
Analysis and Systems Development (ADS) at UniAmérica (Descomplica).

I'm focusing on programming fundamentals, clean code, and building
real-world projects step by step.

---

## 🚀 Main Project

### Investment Platform (Personal Project)
A web-based investment platform designed to explore and analyze investment data.

**Current stage**
- Product ideation and user flow definition
- System architecture planning
- Early backend design

👉 Repository: _coming soon_

---

## 🧪 Academic Projects

- **CS50P Final Project – Expenses Control**
  CLI application focused on expense tracking, price variation analysis,
  and automated testing with pytest.

---

## 📚 Education & Learning

- ADS – UniAmérica (Descomplica)
- CS50P – Harvard (completed)
- CS50X – Harvard (ongoing)

**Core topics**
- Programming fundamentals
- Data structures and algorithms
- Web development basics

---

## 🎯 Current Focus

- Structuring my main project MVP
- Strengthening backend fundamentals with Python
- Applying CS50 concepts to practical projects

---

## 📫 Contact

- Email: lincoln.okoti@gmail.com
- Location: São Paulo, Brazil
